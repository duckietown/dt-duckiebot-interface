"""
    ...
    Configuration:
       dev (str):  Linux joystick device from which to read joystick events, default is `/dev/input/js0`
       deadzone (float): Amount by which the joystick has to move before it is considered to be off-center.
       ...
       autorepeat_rate (float): Rate in Hz at which a joystick that has a non-changing state will resend the
          previously sent message, disabled if 0.0, default is 2.0
       coalesce_interval (float): Axis events that are received within coalesce_interval (seconds) of
