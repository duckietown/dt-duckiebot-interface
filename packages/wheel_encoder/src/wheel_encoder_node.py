...
from wheel_encoder import WheelEncoderDriver, WheelDirection, WheelSide
from duckietown.dtros import DTROS, TopicType, NodeType

# RPi:         1 gpio pin for a wheel encoder, direction from wheel_cmd
# Jetson Nano: 2 gpio pins for a wheel encoder, direction from the encoder
from dt_device_utils import get_device_hardware_brand, DeviceHardwareBrand
ROBOT_HARDWARE = get_device_hardware_brand()
assert ROBOT_HARDWARE in [
    DeviceHardwareBrand.JETSON_NANO,
    DeviceHardwareBrand.RASPBERRY_PI
], "Undefined Hardware!"
...
class WheelEncoderNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        if ROBOT_HARDWARE == DeviceHardwareBrand.RASPBERRY_PI:
            self._gpio_pins = [rospy.get_param('~gpio')]
        elif ROBOT_HARDWARE == DeviceHardwareBrand.JETSON_NANO:
            self._gpio_pins = [
                rospy.get_param('~gpio'),
                rospy.get_param('~gpio2'),
            ]
        ...
        # setup the driver
        self._driver = WheelEncoderDriver(
            self._gpio_pins,
            WheelSide.LEFT if self._configuration == "left" else WheelSide.RIGHT,
            self._encoder_tick_cb,
        )
